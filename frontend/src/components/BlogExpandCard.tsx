...
export default function BlogExpandCard({ post }: BlogExpandCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 top-0 p-4 sm:p-5 [@media(hover:hover)]:transition-opacity [@media(hover:hover)]:duration-500 [@media(hover:hover)]:group-hover/card:opacity-0">
      ...
      <div className="blog-strip__details absolute inset-x-0 bottom-0 translate-y-0 p-4 opacity-100 transition-all duration-500 ease-out sm:p-6 [@media(hover:hover)]:translate-y-3 [@media(hover:hover)]:opacity-0 [@media(hover:hover)]:group-hover/card:translate-y-0 [@media(hover:hover)]:group-hover/card:opacity-100">
        <p className="text-xs font-medium uppercase tracking-wider text-brand-200">{formattedDate}</p>
        <h3 className="mt-2 line-clamp-2 text-lg font-bold leading-tight text-white sm:text-xl md:text-2xl">{post.title}</h3>
        <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-white/85 sm:mt-3 sm:line-clamp-3">{post.excerpt}</p>
        <span className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-white sm:mt-5">
          Read Article
