      ...
      <div className="section-container">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6 lg:gap-3">
          {badges.map(({ icon: Icon, label, tone }) => (
            <div key={label} className="flex min-w-0 items-center gap-2.5 text-ink-muted">
              <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${tone}`}>
                <Icon size={16} strokeWidth={2.25} />
              </div>
              <span className="min-w-0 text-xs font-medium leading-snug text-ink-muted sm:text-sm">{label}</span>
            </div>
          ))}
        </div>
      </div>
